...
//! It also provides an SSE-based HMR endpoint at `/__cloudpack__/hmr` and
//! serves the HMR client script at `/__cloudpack__/hmr-client.js`.
//!
//! ## Root `/` handler
//!
...
//! - Loads the HMR client script (`/__cloudpack__/hmr-client.js`).
...
// ---------------------------------------------------------------------------
// HMR client JavaScript (served at /__cloudpack__/hmr-client.js)
// ---------------------------------------------------------------------------

const HMR_CLIENT_JS: &str = r#"(function() {
  if (typeof EventSource === 'undefined') return;
  const es = new EventSource('/__cloudpack__/hmr');
  es.addEventListener('change', (ev) => { console.log('[cloudpack] change:', ev.data); location.reload(); });
  es.onerror = () => {};
})();"#;
...
<head>
  ...
  <title>Cloudpack Dev — Taskflow</title>
...
<body>
  <div id="root"></div>
  <!-- HMR client: subscribes to /__cloudpack__/hmr SSE and calls location.reload() on change -->
  <script defer src="/__cloudpack__/hmr-client.js"></script>
...
fn transform_on_demand(name: &str, source: &str) -> Result<String> {
    if name.ends_with(".ts") || name.ends_with(".tsx") {
        cloudpack_transform::swc_util::transform_ts_to_js(name, source)
    } else {
        Ok(source.to_string())
    }
}
