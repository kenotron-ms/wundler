...
//! `CLOUDPACK_INTEGRATION_TEST=1` environment variable.

use std::collections::HashMap;
use std::fs;
use std::path::PathBuf;

use cloudpack_transform::engine::BatchConfig;
use cloudpack_transform::rolldown_adapter::RolldownAdapter;
...
// ---------------------------------------------------------------------------
// Integration test (CLOUDPACK_INTEGRATION_TEST=1 only)
// ---------------------------------------------------------------------------

#[test]
fn rolldown_batch_transform_integration() {
    if std::env::var("CLOUDPACK_INTEGRATION_TEST").is_err() {
        // Skip unless opt-in
        return;
    }

    use cloudpack_graph::analyzer::{AnalysisResult, AnalysisStats, GraphAnalyzer};
