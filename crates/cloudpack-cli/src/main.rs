...
use walkdir::WalkDir;
use cloudpack_abs::server::AbsConfig;
use cloudpack_abs::signing::{generate_keypair, ManifestSigner};
use cloudpack_core::{cache::local::LocalCache, validation::run_validate_scale, ModuleSummarizer};
use cloudpack_graph::types::ChunkManifest;
use cloudpack_graph::GraphAnalyzer;
...
use cloudpack_pgo::updater::apply_hints as pgo_apply_hints;
use cloudpack_pipeline::{BuildConfig, BuildPipeline, DevServer, EngineChoice};
...
#[derive(Parser)]
#[command(name = "cloudpack", about = "Cloudpack module bundler tools", version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Summarize {
        ...
        /// Defaults to ~/.cloudpack/cache/summaries/.
    ...
    /// Build the project using cloudpack.toml configuration.
    Build {
        /// Path to the cloudpack.toml configuration file.
...
/// Arguments for `cloudpack pgo ingest`.
#[derive(Parser)]
...
/// Arguments for `cloudpack pgo analyze`.
#[derive(Parser)]
...
/// Arguments for `cloudpack pgo apply`.
#[derive(Parser)]
struct PgoApplyArgs {
    /// Path to the PGO SQLite database.
    #[arg(long, default_value = "pgo.sqlite", value_name = "PATH")]
...
/// Arguments for `cloudpack pgo status`.
#[derive(Parser)]
...
/// Arguments for `cloudpack abs serve`.
#[derive(Parser)]
struct AbsServeArgs {
...
/// Arguments for `cloudpack abs keygen`.
#[derive(Parser)]
...
async fn run_dev(config_path: &Path, port: u16) -> Result<()> {
    ...
    // LocalCache::with_default_root() uses in cloudpack-core/src/cache/local.rs.
...
mod dev_tests {
    ...
    /// The CAS root must be the global `~/.cloudpack/cache/summaries` path,
    ...
    fn cas_root_is_global_summaries_path() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/home/testuser")
                ...
                .join("cache")
                .join("summaries"),
            "CAS root must be $HOME/.cloudpack/cache/summaries"
        );
    }
    ...
    fn cas_root_fallback_dot() {
        ...
        assert_eq!(cas, std::path::PathBuf::from("./.cloudpack/cache/summaries"));
    }
}
...
async fn run_abs_serve(config_path: &Path) -> Result<()> {
    ...
    let config = AbsConfig {
        ...
        security: cloudpack_abs::security::SecurityConfig::default(),
    };

    cloudpack_abs::server::run(config).await
}
...
fn cmd_validate_scale(path: PathBuf, cache_dir: Option<PathBuf>) -> Result<()> {
    ...
    println!();
    println!("┌──────────────────────────────────────────────────────────────┐");
    println!("│ Cloudpack Phase 1 Validation Gate                              │");
    println!("├──────────────────────────────────────────┬───────────────────────┤");
    println!(
        "│ Modules                              │ {:<21} │",
        stats.module_count
...
fn cmd_bench(output: std::path::PathBuf, fast: bool) -> Result<()> {
    ...
    eprintln!("cloudpack bench: analysis benchmark at scales {:?}", scales);
    let analysis = cloudpack_bench::analysis_bench::run(&scales)?;

    eprintln!("cloudpack bench: CAS benchmark at scales {:?}", scales);
    let cas = cloudpack_bench::cas_bench::run(&scales)?;

    let churn_levels = vec![0.005, 0.01, 0.05, 0.10, 0.25, 0.50];
    eprintln!("cloudpack bench: ABS benchmark at scales {:?}", scales);
    let abs_scales = cloudpack_bench::abs_bench::run_scales(&scales, &churn_levels)?;

    cloudpack_bench::report::write_report(&analysis, &cas, &abs_scales, &output)?;
    eprintln!("cloudpack bench: wrote report to {}", output.display());
    Ok(())
}
