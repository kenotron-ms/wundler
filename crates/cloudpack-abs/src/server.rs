...
/// Server configuration for a cloudpack-abs instance.
///
...
/// section is present in `cloudpack.toml`) the middleware is a transparent
...
/// * `X-Cloudpack-Build-Id`: the current manifest's `build_id`
/// * `X-Cloudpack-Signature`: base64-encoded ed25519 signature, if one is stored
...
mod tests {
    ...
    async fn manifest_full_json_returns_manifest_body_and_build_id_header() {
        ...
        assert_eq!(
            resp.headers().get("x-cloudpack-build-id").expect("X-Cloudpack-Build-Id present").to_str().unwrap(),
            "test-build"
        );
    ...
    async fn manifest_full_json_emits_signature_header_when_signature_present() {
        ...
        let resp = router
            ...
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let header = resp.headers().get("x-cloudpack-signature")
            .expect("X-Cloudpack-Signature present").to_str().unwrap().to_string();
        use base64::Engine as _;
    ...
    async fn get_metrics_returns_prometheus_text_format() {
        ...
        assert!(text.contains("cloudpack_manifest_requests_total 5"));
    ...
    async fn get_metrics_shows_chunk_error_after_report() {
        ...
        assert!(text.contains("cloudpack_chunk_errors_total{"));
    ...
    async fn reload_prunes_stale_metrics_entries() {
        ...
        let tmp = tempfile::NamedTempFile::new().expect("tmp manifest file");
        let new_manifest = cloudpack_graph::ChunkManifest {
