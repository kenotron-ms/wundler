...
pub fn render(metrics: &Metrics) -> String {
    ...
    out.push_str("# HELP cloudpack_manifest_requests_total Manifest requests served.\n");
    out.push_str("# TYPE cloudpack_manifest_requests_total counter\n");
    out.push_str(&format!("cloudpack_manifest_requests_total {manifest_requests}\n"));

    // Manifest bytes
    let manifest_bytes = metrics.manifest_bytes_total.load(Ordering::Relaxed);
    out.push_str("# HELP cloudpack_manifest_bytes_total Total bytes served in manifest responses.\n");
    out.push_str("# TYPE cloudpack_manifest_bytes_total counter\n");
    out.push_str(&format!("cloudpack_manifest_bytes_total {manifest_bytes}\n"));

    // Cache hits / misses
    let cache_hits = metrics.cache_hits_total.load(Ordering::Relaxed);
    out.push_str("# HELP cloudpack_cache_hits_total Cache hits.\n");
    out.push_str("# TYPE cloudpack_cache_hits_total counter\n");
    out.push_str(&format!("cloudpack_cache_hits_total {cache_hits}\n"));

    let cache_misses = metrics.cache_misses_total.load(Ordering::Relaxed);
    out.push_str("# HELP cloudpack_cache_misses_total Cache misses.\n");
    out.push_str("# TYPE cloudpack_cache_misses_total counter\n");
    out.push_str(&format!("cloudpack_cache_misses_total {cache_misses}\n"));

    // Chunk errors — one labelled counter per (build_id, chunk_id, error_type)
    out.push_str("# HELP cloudpack_chunk_errors_total Chunk load errors reported by SW.\n");
    out.push_str("# TYPE cloudpack_chunk_errors_total counter\n");
    for entry in metrics.chunk_errors.iter() {
        ...
        out.push_str(&format!(
            "cloudpack_chunk_errors_total{{build_id=\"{build_id}\",chunk_id=\"{chunk_id}\",error_type=\"{error_type}\"}} {val}\n"
        ));
    }
...
mod tests {
    ...
    fn render_includes_manifest_request_counter() {
        ...
        let out = render(&m);
        assert!(out.contains("cloudpack_manifest_requests_total 42"));
    }
    ...
    fn render_includes_chunk_error_with_labels() {
        ...
        let out = render(&m);
        assert!(out.contains("cloudpack_chunk_errors_total{"), "should have labelled metric");
    ...
    fn render_empty_metrics_has_zero_counters() {
        let m = Metrics::new();
        let out = render(&m);
        assert!(out.contains("cloudpack_manifest_requests_total 0"));
        // No chunk_error lines when no errors recorded.
        assert!(!out.contains("cloudpack_chunk_errors_total{"));
    }
