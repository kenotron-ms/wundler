...
(() => {
  // src/cache.ts
  var CACHE_NAME = "cloudpack-v1";
  var STATIC_MANIFEST_KEY = "cloudpack:static-manifest";
  ...
  async function installHandler(config) {
    try {
      ...
      // GET /manifest/full.json with X-Cloudpack-Signature header, but the SW
      ...
      //   2. Read the X-Cloudpack-Signature response header (base64 ed25519)
      ...
      // Until this is done, X-Cloudpack-Signature provides no tamper-evidence.
      const response = await fetch(`${config.cdnBaseUrl}/manifest.json`, {
        cache: "no-cache"
      });
      if (response.ok) {
        const cache = await getCache(config.cache);
        await cache.put("cloudpack:static-manifest", response);
      }
    } catch (err) {
      console.warn("[cloudpack-sw] install: manifest fetch failed", err);
    }
  }
  ...
  if (typeof ServiceWorkerGlobalScope !== "undefined") {
    const CONFIG = {
      cdnBaseUrl: globalThis.__CLOUDPACK_CDN__ ?? "",
      absBaseUrl: globalThis.__CLOUDPACK_ABS__ ?? ""
    };
