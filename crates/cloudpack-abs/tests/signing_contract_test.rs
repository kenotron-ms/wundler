...
fn manifest_signature_bytes_is_deterministic() {
    ...
    let bytes1 = cloudpack_abs::signing::manifest_signature_bytes(&manifest);
    let bytes2 = cloudpack_abs::signing::manifest_signature_bytes(&manifest);
...
fn write_contract_fixtures() {
    let manifest = fixture_manifest();
    let canonical_bytes = cloudpack_abs::signing::manifest_signature_bytes(&manifest);
    let kp = generate_keypair();
    let signer = ManifestSigner::from_pem(&kp.signing_key_pem).expect("signer");
    let sig = signer.sign_manifest(&manifest);

    std::fs::create_dir_all("/tmp/cloudpack-contract").expect("create dir");
    std::fs::write("/tmp/cloudpack-contract/manifest.json", serde_json::to_vec_pretty(&manifest).unwrap()).expect("write manifest");
    std::fs::write("/tmp/cloudpack-contract/canonical_bytes.bin", &canonical_bytes).expect("write bytes");
    std::fs::write("/tmp/cloudpack-contract/signature.bin", sig.to_bytes()).expect("write sig");
    std::fs::write("/tmp/cloudpack-contract/verifying_key.pem", &kp.verifying_key_pem).expect("write key");

    println!("Contract fixtures written to /tmp/cloudpack-contract/");
