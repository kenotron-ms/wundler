...
/// `wundler --help` must list both "build" and "dev" subcommands.
#[test]
fn cli_build_help_lists_subcommands() {
    ...
    assert!(
        ...
        "wundler --help returned non-zero: {:?}\n{combined}",
        output.status
    );
    assert!(
...
/// `wundler build --config <path>` must succeed end-to-end and produce
...
fn cli_build_subcommand_runs_end_to_end() {
    ...
    // Build the wundler.toml with absolute paths.
    ...
    let config_path = dir.path().join("wundler.toml");
    fs::write(&config_path, &config_content).unwrap();

    // Run `wundler build --config <path>`.
    let output = Command::new(wundler_bin())
        .arg("build")
        .arg("--config")
        .arg(&config_path)
