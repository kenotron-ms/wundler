...
//! `wundler abs serve`, `wundler abs keygen`, and `wundler build --sign`.
...
/// `wundler abs keygen --signing-out <path> --verifying-out <path>` must:
/// - exit 0
...
/// `wundler abs serve --help` must exit 0 and advertise the `--config` flag.
#[test]
...
/// `wundler build --help` must advertise the `--sign` flag.
#[test]
