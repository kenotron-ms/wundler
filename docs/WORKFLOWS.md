...
Workflow diagrams for every major user journey in cloudpack. Each diagram is rendered with Mermaid and renders directly in GitHub. Use this document as a map: find the journey that matches what you are doing, then jump to the relevant section of `DEVELOPERS.md` or `CONFIGURATION.md` for detail.

## 1. Local development

The `cloudpack dev` flow. An Axum server starts on the configured port and serves a generated `index.html` that embeds an import map and a small HMR client. TypeScript and JSX files are transformed on demand by SWC with no caching — every request runs the full transform. Bare specifier requests are resolved by trying `.tsx`, `.ts`, `.jsx`, `.js`, and the corresponding `/index.*` variants in order. A file watcher writes change events to an SSE channel; `hmr-client.js` listens and calls `location.reload()` when a relevant path changes.
...
```mermaid
flowchart TD
    A[cloudpack dev --port 3000] --> B[Axum starts on :3000]
    B --> C{Request type}
    ...
    C -->|GET /__cloudpack__/hmr| F[SSE stream open<br/>KeepAlive 30s]
    C -->|GET /__cloudpack__/hmr-client.js| G[Return HMR client script]
...
```mermaid
flowchart TD
    A[Source file changes] --> B[cloudpack build again]
...
sequenceDiagram
    ...
    participant WB as cloudpack build
    participant ABS as cloudpack abs serve
    participant Browser
    participant SW as Service Worker
    participant CDN as Local file server

    Dev->>WB: cloudpack build --config cloudpack.toml
    WB-->>Dev: dist/ (manifest.json, chunks)

    Dev->>ABS: cloudpack abs serve --config abs.toml
...
flowchart TD
    subgraph Build["CI/CD Build"]
        A[Source code] --> B[cloudpack build<br/>--sign --key signing.pem]
        B --> C[chunks/*.js<br/>manifest.json<br/>manifest.sig]
    end

    subgraph Deploy["Deployment"]
        C -->|Upload chunks| CDN[CDN<br/>content-addressed<br/>immutable forever]
        C -->|Update manifest| ABS[ABS Server<br/>cloudpack abs serve<br/>--config abs.toml]
    end
    ...
    subgraph Telemetry["Telemetry Loop"]
        ...
        Log -->|cloudpack pgo ingest| PGO[PGO SQLite store]
        PGO -->|cloudpack pgo apply| ABS
    end
```

## 6. PGO with real user traffic

The production PGO cycle. The ABS writes one JSONL line per `POST /manifest` request. `cloudpack pgo ingest` is idempotent — re-running it on the same log is safe and produces the same store. Once `cloudpack pgo status` reports `READY` (default: ≥100 sessions), the C³ pass computes merge candidates from conditional co-request probabilities, and `cloudpack pgo apply` writes the hints into `manifest.json` atomically. The ABS picks up the new manifest on its next load.
...
flowchart TD
    ...
    D -->|daily/hourly cron| E[cloudpack pgo ingest<br/>telemetry.jsonl --db pgo.sqlite]
    E --> F[PgoStore<br/>SQLite co-request matrix<br/>idempotent: re-ingest safe]

    F --> G{Enough sessions?<br/>Default: 100 min}
    G -->|No — cloudpack pgo status<br/>shows NOT READY| H[Keep collecting]
    G -->|Yes — cloudpack pgo status<br/>shows READY| I[cloudpack pgo analyze<br/>--manifest manifest.json<br/>Preview merge suggestions]

    I --> J{Suggestions acceptable?}
    J -->|No| H
    J -->|Yes| K[cloudpack pgo apply<br/>--db pgo.sqlite<br/>--manifest dist/manifest.json]
...
flowchart TD
    ...
    C --> D[cloudpack pgo ingest<br/>synthetic.jsonl --db pgo.sqlite]
    D --> E[PgoStore populated<br/>with simulated sessions]

    E --> F[cloudpack pgo apply<br/>--db pgo.sqlite --manifest manifest.json]
...
>
> Simulated sessions let you bootstrap PGO hints before launch. The `merge_threshold` default (0.70) is calibrated for real traffic. Synthetic sessions often produce P(b|a) = 1.0 for co-requested chunks — this saturates the threshold and may over-merge. Consider using `merge_threshold: 0.90` when working with synthetic data, and re-run `cloudpack pgo apply` after your first 1,000 real sessions.
>
> The `cloudpack pgo status` command shows `READY/NOT READY` based on `min_sessions` (default: 100). For synthetic bootstrapping, you may want to lower this to 10 or 20 and raise it in production.

## 8. Manifest signing and verification

Ed25519 keys are generated once with `cloudpack abs keygen`. The signing key (`signing.pem`) is a CI secret; the verifying key (`verifying.pem`) is embedded in the ABS config and, eventually, in the Service Worker. Every build calls `cloudpack build --sign --key signing.pem` which writes `manifest.sig` next to `manifest.json`. At startup the ABS verifies the signature and refuses to serve if the file has been tampered.
...
sequenceDiagram
    ...
    Ops->>Ops: cloudpack abs keygen<br/>--signing-out signing.pem<br/>--verifying-out verifying.pem

    Note over Ops: signing.pem → CI secret<br/>verifying.pem → abs.toml + SW

    Note over Ops: Every build
    Ops->>WB: cloudpack build<br/>--sign --key signing.pem
