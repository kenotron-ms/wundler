...
Before proposing new work, we examined an existing internal project (Cloudpack) that set out to solve a closely related problem at comparable scale. Cloudpack independently arrived at three of the conclusions central to Cloudpack's design: pre-compute lightweight metadata before any bundling work, decompose the application into independently-buildable units, and stitch results together at runtime rather than at build time. Cloudpack validates the architectural direction. Cloudpack extends it by solving the production-delivery problem that Cloudpack explicitly deferred and by operating at finer granularity, which preserves correctness guarantees that Cloudpack trades away for speed.

## The Approach

Cloudpack is structured as three phases that operate on a continuously-maintained module graph, plus an optional adaptive delivery layer.
...
The transform engine itself is pluggable. Cloudpack does not reinvent the production-grade bundler; it drives Rolldown or Rspack underneath, inheriting their correctness and their plugin ecosystems. The innovation is in everything above that layer.
...
Correctness risk is contained by leaning on Rolldown or Rspack for the actual code transformation. Cloudpack controls the orchestration and graph analysis; it does not rewrite JavaScript semantics. A correctness audit mode is built in from the start, comparing Cloudpack's output against a known-good baseline before any developer-facing rollout.
