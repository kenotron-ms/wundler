...
- `wundler-graph/src/manifest.rs` already has a `compute_build_id()` that hashes entry-point **route paths** → 64-char hex. This is the **old** computation set by the graph layer. The pipeline will **override** it.
- `ContentHash` (`wundler-core/src/types.rs`) derives `Hash + Eq` but **not `Ord`** — use `BTreeMap<&str, _>` (via `.0.as_str()`) instead of `BTreeMap<&ContentHash, _>`.
...
- `axum-test = "20"` and `tempfile = "3"` are already in `wundler-abs` dev-dependencies.
...
**Files:**
- Create: `crates/wundler-pipeline/src/build_id.rs`
...
In `crates/wundler-pipeline/src/lib.rs`, add `pub mod build_id;` as the first line:
...
Run:
```
cargo build -p wundler-pipeline 2>&1 | head -20
```
...
**Files:**
- Create: `crates/wundler-pipeline/tests/build_id_test.rs`
...
Run:
```
cargo test -p wundler-pipeline --test build_id_test 2>&1 | tail -20
```
...
**Files:**
- Modify: `crates/wundler-pipeline/src/build_id.rs`

**Step 1: Write the full implementation**

Replace the entire content of `crates/wundler-pipeline/src/build_id.rs` with:
...
Run:
```
cargo test -p wundler-pipeline --test build_id_test -- --nocapture 2>&1
```
...
```
git add crates/wundler-pipeline/src/build_id.rs \
        crates/wundler-pipeline/src/lib.rs \
        crates/wundler-pipeline/tests/build_id_test.rs
...
**Files:**
- Create: `crates/wundler-pipeline/tests/build_id_pipeline_test.rs`
...
Run:
```
cargo test -p wundler-pipeline --test build_id_pipeline_test 2>&1 | tail -15
```
...
In `crates/wundler-pipeline/src/pipeline.rs`, make two changes:
...
Run:
```
cargo test -p wundler-pipeline --test build_id_pipeline_test -- --nocapture 2>&1
```
...
```
cargo test -p wundler-pipeline 2>&1 | tail -10
```

Expected: All tests pass. The `wundler-graph` tests that check for 64-char build_ids still pass because they test `wundler_graph::manifest::compute_build_id` (the old function, untouched).

**Step 6: Commit**

```
git add crates/wundler-pipeline/src/pipeline.rs \
        crates/wundler-pipeline/tests/build_id_pipeline_test.rs
...
Append to the end of `crates/wundler-abs/tests/state_test.rs`:
...
Run:
```
cargo test -p wundler-abs --test state_test reload_manifest 2>&1 | tail -15
```
...
In `crates/wundler-abs/src/state.rs`, add the following method to the `impl AppState` block, after `load_from_disk`:
...
Run:
```
cargo test -p wundler-abs --test state_test 2>&1 | tail -10
```
...
In `crates/wundler-abs/src/server.rs`, apply the following changes:
...
Run:
```
cargo build -p wundler-abs 2>&1 | head -20
```
...
**Files:**
- Create: `crates/wundler-abs/tests/reload_test.rs`
- Modify: `crates/wundler-abs/src/server.rs`
...
Run:
```
cargo test -p wundler-abs --test reload_test test_reload_loads_new_manifest 2>&1 | tail -10
```
...
Run:
```
cargo test -p wundler-abs --test reload_test test_reload_loads_new_manifest -- --nocapture 2>&1
```
...
**Files:**
- Modify: `crates/wundler-abs/tests/reload_test.rs`
- Modify: `crates/wundler-abs/src/server.rs`

**Step 1: Write the error-path tests**

Append to `crates/wundler-abs/tests/reload_test.rs`:
...
Run:
```
cargo test -p wundler-abs --test reload_test 2>&1 | tail -15
```
...
```
cargo test -p wundler-abs --test reload_test -- --nocapture 2>&1
```
...
In `crates/wundler-abs/src/server.rs`, find the `run()` function and update the `axum::serve` call:
...
**Step 4: Run the full `wundler-abs` test suite**

Run:
```
cargo test -p wundler-abs 2>&1 | tail -15
```
...
- [ ] `cargo test -p wundler-pipeline --test build_id_test` → 4 passed
- [ ] `cargo test -p wundler-pipeline --test build_id_pipeline_test` → 3 passed
- [ ] `cargo test -p wundler-abs --test state_test` → 6 passed (4 existing + 2 new)
- [ ] `cargo test -p wundler-abs --test reload_test` → 3 passed
...
| **Create** | `crates/wundler-pipeline/tests/build_id_pipeline_test.rs` |
| **Create** | `crates/wundler-abs/tests/reload_test.rs` |
| **Modify** | `crates/wundler-pipeline/src/lib.rs` — add `pub mod build_id;` |
| **Modify** | `crates/wundler-pipeline/src/pipeline.rs` — `let mut analysis`, call `compute_build_id` |
| **Modify** | `crates/wundler-abs/src/state.rs` — add `reload_manifest` method |
| **Modify** | `crates/wundler-abs/src/server.rs` — imports, types, route, handler, `run()` |
| **Modify** | `crates/wundler-abs/tests/state_test.rs` — add 2 reload_manifest tests |
